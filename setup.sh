...
# ─── browse — One-Command Setup ─────────────────────────────────────────
# Downloads Tor Browser + geckodriver, installs the Python package,
...
echo -e "\n${BOLD}browse — Anti-Fingerprint AI Browser${NC}\n"
...
TBB_DIR="$RUNTIME_DIR/tor-browser"
TBB_ARCHIVE="$RUNTIME_DIR/tor-browser-linux-x86_64-${TBB_VERSION}.tar.xz"
TBB_URL="https://dist.torproject.org/torbrowser/${TBB_VERSION}/tor-browser-linux-x86_64-${TBB_VERSION}.tar.xz"

if [ -d "$TBB_DIR" ] && [ -f "$TBB_DIR/Browser/firefox" ]; then
    ok "Tor Browser ${TBB_VERSION} already installed"
else
    if [ -f "$TBB_ARCHIVE" ]; then
        info "Tor Browser archive already downloaded"
    else
        info "Downloading Tor Browser ${TBB_VERSION} (~130 MB)..."
        curl -L --progress-bar -o "$TBB_ARCHIVE" "$TBB_URL" || \
            fail "Failed to download Tor Browser. Check your internet connection."
        ok "Downloaded Tor Browser"
    fi

    info "Extracting Tor Browser..."
    tar -xf "$TBB_ARCHIVE" -C "$RUNTIME_DIR" || \
        fail "Failed to extract Tor Browser archive"

    if [ ! -d "$TBB_DIR" ]; then
        EXTRACTED=$(find "$RUNTIME_DIR" -maxdepth 1 -type d -name "tor-browser*" | head -1)
        if [ -n "$EXTRACTED" ] && [ "$EXTRACTED" != "$TBB_DIR" ]; then
            mv "$EXTRACTED" "$TBB_DIR"
        fi
    fi

    if [ ! -f "$TBB_DIR/Browser/firefox" ]; then
        fail "Tor Browser extraction failed — firefox binary not found"
    fi

    ok "Tor Browser ${TBB_VERSION} installed"
    rm -f "$TBB_ARCHIVE"
fi
